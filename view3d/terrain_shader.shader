...
mat3 get_triplanar_uv(vec3 position, vec3 normal) {
	...
	uvs[0] = vec3(-position.zy, 0.0) / triplanar_scale;
	uvs[1] = vec3(-position.xz, 0.0) / triplanar_scale;
	uvs[2] = vec3(-position.xy, 0.0) / triplanar_scale;
...
vec3 _fragment(vec2 uv) {
    vec3 color = sample(base, uv);
    ...
    if (abs(slope_dot) >= flat_land_dot - flat_land_falloff) {
        ...
        color = mix(color, sample(flat_land, uv), falloff);
    }
    if (height <= sand_height + sand_falloff) {
        ...
        color = mix(color, sample(sand, uv), falloff);
    }
    if (height >= snow_height - snow_falloff) {
        ...
        color = mix(color, sample(snow, uv), falloff);
    }
